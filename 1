...
local Window = Library:CreateWindow({
    Title            = "Kanzyu_script",
    Footer           = "Violence District - YT kanzyu _script",
    Icon             = 91112706169806,
    IconSize         = UDim2.fromOffset(40, 40),
    ShowHeaderGlow   = true,
    CornerRadius     = 12,  -- Rounded corners (lebih melengkung)
    NotifySide       = "Right",
    ...
    GlobalSearch     = false,
    EnableSidebarResize = false,  -- Nonaktifkan resize sidebar
    EnableCompacting = true,
    SidebarCompacted = true,
    -- Kunci ukuran window
    MinSize          = UDim2.fromOffset(400, 600),
    MaxSize          = UDim2.fromOffset(400, 600),
    Resizable        = false,
})
-- TABS
local Tabs = {
    Info       = Window:AddTab("Info",        "info","Information, Credit"),
...
local function createEmoteButton()
    ...
    btn.Image = "rbxassetid://93349170559446"; btn.ImageTransparency = 0.1; btn.Parent = gui
    local corner = Instance.new("UICorner"); corner.CornerRadius = UDim.new(1,0); corner.Parent = btn
    local stroke = Instance.new("UIStroke")
    stroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
...
InfoBox:AddLabel("Dev: •༶Kanzyu༶•")
InfoBox:AddLabel("Join discord for more info")
InfoBox:AddLabel("Discord:")
InfoBox:AddButton("Copy Discord Link", function()
    setclipboard("https://discord.gg/3uuu28XTE")
    Library:Notify({Title = "Discord link copied!", Duration = 3})
end)

CreditsBox:AddLabel("Developer:")
CreditsBox:AddLabel("• Kanzyu_script")
...
CreditsBox:AddButton("Copy Support Link", function()
    setclipboard("https://saweria.co/kanzyuscrpt")
    Library:Notify({Title = "Thanks for the support!", Duration = 3})
end)
...
AbilityTab:AddCheckbox("GenBoostToggle", {
    Text = "GenBoost (All Gens)",
    Default = false,
    Tooltip = "perk perfectionis aktif di SEMUA generator tanpa limit.",
    Callback = function(Value)
        GenBoostConfig.Enabled = Value
        if not Value then
            local perksFolder = Remotes:FindFirstChild("Perks")
            local perfRemote = perksFolder and perksFolder:FindFirstChild("perfectionistplanning")
            if perfRemote then
                pcall(function() perfRemote:FireServer("clearBoost") end)
            end
        end
    end
})
...
AbilityTab:AddLabel("Moonwalk Keybind"):AddKeyPicker("MoonwalkKey", {
    ...
    Callback = function(state)
        ...
        if state then
            ...
            if Connections.Moonwalk then Connections.Moonwalk:Disconnect(); Connections.Moonwalk = nil end
            if hum then hum.WalkSpeed = Movement.WalkSpeedEnabled and Movement.WalkSpeedValue or Movement.OriginalWalkSpeed end
        end
    end
})
...
ThemeManager:SetFolder("Kanzyu_script")
SaveManager:SetFolder("Kanzyu_script/configs")
